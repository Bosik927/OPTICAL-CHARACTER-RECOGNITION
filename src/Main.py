...
root = Tk()
img = ImageTk.PhotoImage(Image.open("src\download.png"))

createRect = Label(root, text="Read image: ")
createRect.grid(row =0, column=0)


leftImage = Label(root, image = img)
leftImage.grid(row =1, column=0)
leftImage.bind('<Enter>', createRectangle)
leftImage.bind("<Leave>", disappearRectangle)
